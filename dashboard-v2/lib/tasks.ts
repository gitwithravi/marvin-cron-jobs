    ...
    const tasks = await Promise.all(
      entries
        ...
        .map(async (entry): Promise<TaskSummary | null> => {
          ...
          const runs = (await res.json()) as TaskRun[];
          const reports: ReportSummary[] = runs.map((run, index) => {
            const timeVal = run.observed_at || run.started_at;
            return {
              fileName: String(run.id),
              label: formatDateLabel(timeVal),
              href: `/console/runs/${encodeURIComponent(taskName)}?run=${run.id}`,
              modifiedAt: timeVal,
              isLatest: index === 0,
...
): Promise<{ task: TaskSummary; selectedReport: ReportSummary | null; run: TaskRun | null } | null> {
