      ...
      entries
        ...
        .map(async (entry): Promise<TaskSummary | null> => {
          ...
          return {
            ...
            latestReport,
            reports,
            riskLevel: latestReport?.riskLevel || null
          };
        })
    );
...
export async function getReportDetail(
  ...
  const selectedReport = requestedReport
    ? task.reports.find((report) => report.fileName === requestedReport) || null
    : task.latestReport;

  if (!selectedReport) {
    return { task, selectedReport: null, run: null };
  }
  ...
  try {
    ...
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) {
      return { task, selectedReport, run: null };
    }
    return { task, selectedReport, run: await res.json() };
  } catch (error) {
