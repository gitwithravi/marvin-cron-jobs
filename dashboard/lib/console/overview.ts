import { fetchMarvinApi } from "@/lib/api/console";
import { getOpenRouterAccountUsage } from "@/lib/openrouter-usage";
import { getTasks, type TaskSummary } from "@/lib/tasks";
import { normalizeConsoleTone } from "@/lib/console/status";
...
function sortTasksByUrgency(tasks: TaskSummary[]): TaskSummary[] {
  const weight = (riskLevel: string | null): number => {
    switch (normalizeConsoleTone(riskLevel)) {
      case "critical":
        return 5;
      case "failed":
        return 4;
  ...
  return [...tasks].sort((left, right) => {
    const byRisk = weight(right.riskLevel) - weight(left.riskLevel);
    if (byRisk !== 0) {
      return byRisk;
    }
...
function taskConclusion(task: TaskSummary): string {
  ...
  switch (normalizeConsoleTone(task.riskLevel)) {
    case "critical":
...
function buildHeadline(tasks: TaskSummary[], pendingApprovals: ApprovalSummary[]): string {
  const criticalCount = tasks.filter((task) => normalizeConsoleTone(task.riskLevel) === "critical").length;
  const warningCount = tasks.filter((task) => normalizeConsoleTone(task.riskLevel) === "warning").length;
...
export async function getConsoleOverviewData(): Promise<ConsoleOverviewData> {
  ...
  const prioritized = sortTasksByUrgency(tasks).map((task) => ({
    ...
    href: task.latestReport?.href || `/console/reports/${encodeURIComponent(task.taskName)}`,
  ...
  return {
    ...
    alertMessage:
      alert.message || "Latest alert has not been generated yet. Tranquility may be temporary.",
